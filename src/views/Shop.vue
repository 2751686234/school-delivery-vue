  ...
  <div class="shop-page">
    ...
    <div class="container">
      ...
      <div v-if="currentShop.id" class="evaluate-section">
        <h3 class="text-center sub-title">店铺评价</h3>
        <el-card v-if="evaluateList.length === 0" style="text-align:center;padding:20px;color:#909399">暂无评价</el-card>
        <div v-else class="evaluate-list">
          <el-card v-for="ev in evaluateList" :key="ev.id" class="evaluate-item">
            <div class="ev-header">
              <el-rate v-model="ev.score" disabled show-score text-color="#ff9900" score-template="{value}分" />
              <span class="ev-time">{{ formatTime(ev.createTime) }}</span>
            </div>
...
// 获取商家列表
const getShopList = () => {
  request.get('/shop/list').then(res => { shopList.value = res.data })
}
...
const loadEvaluates = async (shopId) => {
  try {
    const res = await request.get('/evaluate/list', { params: { shopId } })
    evaluateList.value = res.data || []
  } catch (e) { evaluateList.value = [] }
}
...
.shop-img { width: 100%; height: 140px; border-radius: 12px 12px 0 0; object-fit: cover; margin: -16px -16px 12px -16px; }
.shop-name { margin-top: 8px; font-weight: 700; font-size: 16px; color: #2a3340; }
.food-section { margin-top: 32px; }
.sub-title { font-size: 22px; font-weight: 800; color: #ff6b35; margin-bottom: 20px; letter-spacing: 1px; }
.food-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px; }
.food-card { border-radius: 16px; border: none; box-shadow: 0 6px 18px rgba(0,0,0,0.08); transition: all 0.3s; overflow: hidden; cursor: default; }
